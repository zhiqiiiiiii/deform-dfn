...
class BouncingMnistDataset(Dataset):
    def __init__(self, f, background='zeros', mode='standard', num_frames=20, image_size=64, num_digits=2, step_length=0.1):
#         mnistDataset='data/mnist.h5'
#         try:
...
#             sys.exit()
    ...
    def __getitem__(self, idx):
        ...
        if self.background_ == 'zeros':
            data = np.zeros((self.num_channels_, self.image_size_, self.image_size_, self.seq_length_), dtype=np.float32)
        elif self.background_ == 'rand':
            data = np.random.rand(self.num_channels_, self.image_size_, self.image_size_, self.seq_length_)
        ...
        for n in range(self.num_digits_):
            ...
            for i in range(self.seq_length_):
                ...
                data[:, top:bottom, left:right, i] = self.Overlap(data[:, top:bottom, left:right, i], digit_image)
                
        #     frames = np.transpose(frames, [0,2,3,1,4]) # (batch, h, w, c, frames)
        data = torch.from_numpy(data) #C, H, W, frame
        return data

#uncomment to test
#from datasets.BouncingMnistDataset import BouncingMnistDataset
# dh = BouncingMnistDataset()
# frames = dh[0]
# print(frames.shape)
